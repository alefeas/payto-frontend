...
export const FluidGradient = ({ color1, color2, color3, color4 }: FluidGradientProps = {}) => {
  const [simKey, setSimKey] = useState(0);

  useEffect(() => {
    // Update config colors if props are provided
    if (color1) config.color1 = color1;
    if (color2) config.color2 = color2;
    if (color3) config.color3 = color3;
    if (color4) config.color4 = color4;
